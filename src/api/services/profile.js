...
export const changePassword = async (credentials) => {
  const res = await axios.post(`${url}/auth/change-password/`, credentials, {
    headers: {
      Authorization: `Bearer ${getLoginToken()}`,
    },
  })
...
export const verifyUser = async (userData) => {
  const res = await axios.post(`${url}/account/profiles/verify/`, userData, {
    headers: {
      Authorization: `Bearer ${getLoginToken()}`,
      'Content-Type': 'multipart/form-data',
    },
...
export const unstickUser = async (userId) => {
  const res = await axios.get(`${url}/account/profiles/${userId}/unstick/`, {
    headers: {
      Authorization: `Bearer ${getLoginToken()}`,
    },
  })
...
export const updateGadget = async (id, info) => {
  const res = await axios.patch(`${url}/account/profile/phones/${id}/`, info, {
    headers: {
      Authorization: `Bearer ${getLoginToken()}`,
      'Content-Type': 'multipart/form-data',
    },
